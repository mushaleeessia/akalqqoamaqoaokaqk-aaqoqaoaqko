...
-- Adicionar suporte para palavras cruzadas nas tabelas existentes
-- Primeiro, vamos adicionar 'cruzadas' como um modo de jogo válido
-- Não precisamos criar novas tabelas, apenas usar as existentes

-- Verificar se já existem registros com game_mode 'cruzadas' e adicionar se necessário
-- (As tabelas game_sessions e game_stats já suportam qualquer string como game_mode)

-- Função para validar se um puzzle de palavras cruzadas está completo
CREATE OR REPLACE FUNCTION public.is_crossword_complete(
  grid_data jsonb,
  target_words jsonb
) RETURNS boolean
LANGUAGE plpgsql
